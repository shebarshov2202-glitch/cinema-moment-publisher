...
export default async function handler(req) {
  ...
  try {
    ...
    if (!accessToken) {
      return json({ ok: false, error: 'access_token is required' }, 400);
    }

    if (ownerId !== -GROUP_ID) {
      return json({ ok: false, error: 'owner_id is not allowed' }, 403);
    }

    if (!message && !attachments) {
      return json({ ok: false, error: 'Публикация пустая' }, 400);
    }
